...
<c:choose>
	<c:when test="${not empty searchVO.boardId}">
		<c:set var="actionUrl" value="/notice/update.do" />
	</c:when>
	<c:otherwise>
		<c:set var="actionUrl" value="/notice/insert.do" />
	</c:otherwise>
</c:choose>
		...
		<div class="btn-cont ar">
			<c:choose>
				<c:when test="${not empty searchVO.boardId}"> <!-- Controller에서 //본인 및 관리자만 허용하게 했기 때문에 -->
					<c:url var="uptUrl" value="/notice/update.do">
			...
			<c:url var="listUrl" value="/notice/selectList.do" />
